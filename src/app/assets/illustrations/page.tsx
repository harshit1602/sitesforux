    ...
    <div className="w-full flex flex-col main-content gap-8 bg-pagebg lg:rounded-xl h-full overflow-y-auto">
      ...
      <header className="flex flex-col lg:flex-row items-start gap-5 lg:gap-4 lg:justify-between px-4 lg:px-7 pt-8 pb-4">
        <div>
          ...
          Find Inspiring Illustrations for <br/> Your Creative Projects
          </h1>
          <h2 className="text-[#707070] text-sm lg:text-lg font-instrument font-normal mt-2 lg:mt-4 leading-tight">
          Discover platforms offering premium and free illustrations and to bring your designs to life
          </h2>
        </div>
      ...
      <section className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-4 px-4 lg:px-7">
        {illustrations.map((illustrations) => (
          <ImageCard
            key={illustrations.id}
            src={illustrations.image}
            alt={illustrations.name}
            name={illustrations.name}
            link={illustrations.link}
            isEditorsChoice={illustrations.isEditorsChoice}
            isCategoryAvailable={illustrations.isCategoryAvailable}
