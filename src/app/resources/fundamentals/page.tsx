...
import { fundamentals } from "@/data/resources";
    ...
    <div className="w-full flex flex-col main-content gap-8 bg-pagebg lg:rounded-xl h-full overflow-y-auto">
      ...
      <header className="flex flex-col lg:flex-row items-start gap-5 lg:gap-4 lg:justify-between px-4 lg:px-7 pt-8 pb-4">
        <div>
          ...
          Master UX Design Fundamentals <br/> with Curated Learning Resources
          </h1>
          <h2 className="text-[#707070] text-sm lg:text-lg font-instrument font-normal mt-2 lg:mt-4 leading-tight">
          Master the core principles of design with resources <br/> that build a strong foundation for UI/UX success
          </h2>
        </div>
      ...
      <section className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-4 px-4 lg:px-7">
        {fundamentals.map((fundamentals) => (
          <ImageCard
            key={fundamentals.id}
            src={fundamentals.image}
            alt={fundamentals.name}
            name={fundamentals.name}
            link={fundamentals.link}
            isEditorsChoice={fundamentals.isEditorsChoice}
            isCategoryAvailable={fundamentals.isCategoryAvailable}
