...
import { prodtools } from "@/data/tools";
import NewsletterModal from "@/components/newslettermodal";
import SubmitModal from "@/components/submitmodal";

export default function ProdTools() {
    ...
    <div className="w-full flex flex-col main-content gap-8 bg-pagebg lg:rounded-xl h-full overflow-y-auto">
      ...
      <header className="flex flex-col lg:flex-row items-start gap-5 lg:gap-4 lg:justify-between px-4 lg:px-7 pt-8 pb-4">
        <div>
          ...
          Optimize Your Efficiency with <br/> Top Productivity Tools
          </h1>
          <h2 className="text-[#707070] text-sm lg:text-lg font-instrument font-normal mt-2 lg:mt-4 leading-tight">
          Explore tools to organize tasks, manage time, and improve <br/> team collaboration to maximize your productivity
          </h2>
        </div>
      ...
      <section className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-4 px-4 lg:px-7">
        {prodtools.map((prodtools) => (
          <ImageCard
            ...
            name={prodtools.name}
            link={prodtools.link}
            isEditorsChoice={prodtools.isEditorsChoice}
            isCategoryAvailable={prodtools.isCategoryAvailable}
            category={prodtools.category}
          />
        ))}
      </section>
