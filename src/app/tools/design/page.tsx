...
import { designtools } from "@/data/tools";
    ...
    <div className="w-full flex flex-col main-content gap-8 bg-pagebg lg:rounded-xl h-full overflow-y-auto">
      ...
      <header className="flex flex-col lg:flex-row items-start gap-5 lg:gap-4 lg:justify-between px-4 lg:px-7 pt-8 pb-4">
        <div>
          ...
          Empower Your Creativity with <br/> Essential Design Tools
          </h1>
          <h2 className="text-[#707070] text-sm lg:text-lg font-instrument font-normal mt-2 lg:mt-4 leading-tight">
          Uncover the best design tools from prototyping to wireframing to bring your ideas to life effortlessly
          </h2>
        </div>
      ...
      <section className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-4 px-4 lg:px-7">
        {designtools.map((designtools) => (
          <ImageCard
            key={designtools.id}
            src={designtools.image}
            alt={designtools.name}
            name={designtools.name}
            link={designtools.link}
            isEditorsChoice={designtools.isEditorsChoice}
            isCategoryAvailable={designtools.isCategoryAvailable}
